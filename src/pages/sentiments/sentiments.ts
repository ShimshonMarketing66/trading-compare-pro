...
export class SentimentsPage {
  ...
  constructor(
    ...
    this.globalProvider.get_sentiments().then((sentiments_from_server: {
      type: string, symbol_type: string, price: number, user_id: string, symbol: string, date: string, status: string
    }[]) => {
      console.log("sentiment from server", sentiments_from_server);
      let promises = [];
  ...
  close(item, i) {
    ...
    this.globalProvider.close_sentiment(item.symbol, item.type_sentiment, item.price);
  }
