import { Component, ViewChild, AfterViewChecked, ChangeDetectionStrategy, NgZone, Input } from '@angular/core'
...
export class ChartUI implements AfterViewChecked, OnInit {
  ...
  changeChartType(type) {
    ...
    if ((type.aggregationEdit && this.chartComponent.ciq.layout.aggregationType != type.type) || type.type == 'heikinashi') {
      ...
      this.chartComponent.ciq.setAggregationType(type);
    } else {
  ...
  onSelectPeriod(newperiodicity) {
    this.periodicity = newperiodicity;
    for (let index = 0; index < this.periodicityOptions.length; index++) {
      if (this.periodicity == this.periodicityOptions[index].label) {
        this.changePeriodicity(this.periodicity, this.periodicityOptions[index].interval)
        break
      }
    }
  ...
  onSelectType(chartType) {
    ...
    for (let index = 0; index < this.chartTypes.length; index++) {
     if (this.chartTypes[index].label == chartType ) {
